...
impl LcdBackend for EpdLcd {
    const WIDTH: u16 = EPD_WIDTH;
    const HEIGHT: u16 = EPD_HEIGHT;
    ...
    /// Ghost-erase before a full redraw: drive every pixel white so prior dark
    /// content does not bleed through the next frame. Wired into the buffered /
    /// full-redraw path in `vm.rs` / `render.rs`.
