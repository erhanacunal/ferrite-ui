...
fn peek_phys_bytes(phys: u32, buf: &mut [u8]) -> bool {
    ...
    let scratch_bytes =
        unsafe { slice::from_raw_parts(scratch.as_ptr() as *const u8, SCRATCH_BYTES) };
    buf.copy_from_slice(&scratch_bytes[prefix..prefix + payload]);
    true
}

// `Clone` so `Platform::FlashB` is satisfied — the VM takes its own flash
// handle for flash-execution cache refills. The backend is a stateless XIP
// mapper, so the clone shares no mutable state.
#[derive(Clone, Copy)]
pub struct EspFlash;
