...
static ROW2_LOWER: [KeyDef; 9] = [
    ...
    KeyDef::new(102, 68, b'z'), KeyDef::new(174, 68, b'x'), KeyDef::new(246, 68, b'c'),
    KeyDef::new(318, 68, b'v'), KeyDef::new(390, 68, b'b'), KeyDef::new(462, 68, b'n'),
...
static ROW2_SYMBOLS: [KeyDef; 9] = [
    KeyDef::new(2, 96, KEY_SHIFT),
    KeyDef::new(102, 68, b'*'), KeyDef::new(174, 68, b'"'), KeyDef::new(246, 68, b'\''),
    KeyDef::new(318, 68, b':'), KeyDef::new(390, 68, b';'), KeyDef::new(462, 68, b'!'),
    KeyDef::new(534, 68, b'?'),
    KeyDef::new(606, 192, KEY_BACKSPACE),
];
...
impl Keyboard {
    ...
    fn key_at(&self, x: u16, y: u16) -> Option<u8> {
        ...
        match self.layer {
            0 => self.search_row(x, row_idx, &ROW0_LOWER, &ROW1_LOWER, &ROW2_LOWER, &ROW3_LOWER),
            1 => self.search_row(x, row_idx, &ROW0_UPPER, &ROW1_UPPER, &ROW2_UPPER, &ROW3_UPPER),
            _ => self.search_row(x, row_idx, &ROW0_SYMBOLS, &ROW1_SYMBOLS, &ROW2_SYMBOLS, &ROW3_SYMBOLS),
        }
    }
    ...
    pub fn draw(&self, lcd: &Lcd, fonts: &FontList, flash: &Flash) {
        if !self.visible {
            return;
        }
        ...
        match self.layer {
            0 => {
                self.draw_rows(lcd, font, flash, &ROW0_LOWER, &ROW1_LOWER, &ROW2_LOWER, &ROW3_LOWER);
            }
            1 => {
                self.draw_rows(lcd, font, flash, &ROW0_UPPER, &ROW1_UPPER, &ROW2_UPPER, &ROW3_UPPER);
            }
            _ => {
                self.draw_rows(lcd, font, flash, &ROW0_SYMBOLS, &ROW1_SYMBOLS, &ROW2_SYMBOLS, &ROW3_SYMBOLS);
            }
        }
    }
